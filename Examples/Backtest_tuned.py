...
def train_regime_aware_model(train_dataset, val_dataset, config):
    """
    ...
    config.model.transformer_dropout = 0.3
    ...
    for epoch in range(20):
        ...
        for X, y in train_loader:
            optimizer.zero_grad()
            predictions, metadata = model(X, return_components = True)
        ...
        with torch.no_grad():
            for X, y in val_loader:
                pred, metadata = model(X, return_components = True)
...
def generate_regime_aware_signals(model, test_features, symbols, sequence_length=252):
    """
    ...
    with torch.no_grad():
        for symbol in symbols:
            ...
            for i in range(sequence_length, len(symbol_data)):
                ...
                prediction, metadata = model(X, return_components=True)
            ...
            # Average signal for this stock
            signals[symbol] = np.median(symbol_predictions) if symbol_predictions else 0
    ...
    if signals:
        ...
        print("⚠️ No signals generated (insufficient data after filtering)")
...
def calculate_performance_metrics_with_rebalancing(returns: pd.Series, 
    ...
    if rebalance_frequency == 'daily':
        ...
        if rebalance_mask.sum() > 0:
            # Further filter to only first Monday per month
            monthly_mondays = returns[rebalance_mask].resample('MS').first()
            rebalance_dates = []
            for month_start in monthly_mondays.index:
                # Find first Monday in this month
                month_data = returns[rebalance_mask][returns[rebalance_mask].index.month == month_start.month]
                if len(month_data) > 0:
                    rebalance_dates.append(month_data.index[0])
            rebalance_mask = returns.index.isin(rebalance_dates)
        else:
            # Fallback: first trading day of each month
            monthly_dates = returns.resample('MS').first().index
            rebalance_mask = returns.index.isin(monthly_dates)
    else:
    ...
    if num_rebalances == 0:
        print(f"\n⚠️  WARNING: Found 0 rebalances for {rebalance_frequency} frequency!")
    ...
    for i, (timestamp, ret) in enumerate(returns.items()):
        ...
        # Calculate net return for this period
        if i == 0:
            net_return = ret - (cost_per_rebalance * 2 if rebalance_mask.iloc[i] else 0)
        else:
            prev_value = 1.0
            for j in range(i):
                prev_value *= (1 + returns.iloc[j])
                if rebalance_mask.iloc[j]:
                    prev_value *= (1 - cost_per_rebalance * 2)
            net_return = (portfolio_value - prev_value) / prev_value
        
        net_returns.append(ret if not rebalance_mask.iloc[i] else ret - cost_per_rebalance * 2)
    
    net_returns_series = pd.Series(net_returns, index=returns.index)
...
def plot_equity_curves(portfolio_returns: pd.DataFrame, 
    ...
    rebalance_configs = {
        'optimistic': {'bps': 5, 'freq': 'weekly'},
        'realistic': {'bps': 10, 'freq': 'weekly'},
        'pessimistic': {'bps': 20, 'freq': 'weekly'}
    }
...
def save_performance_summary(metrics_by_cost: Dict[str, Dict],
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write(f"  R^2:                   {prediction_metrics['r2']:.4f}\n")
        f.write(f"  Note: Negative R Squared is normal for ranking strategies. Use IC for ranking quality. \n")
...
def main():
    ...
    MAX_SYMBOLS = None  # Start with 20, set to None for all 100
    MAX_POSITIONS = 10
    ...
    print("\n[Step 3/7] Calculating Actual Returns...")
    actual_returns_list = []
    for symbol in symbols:
    ...
    print(f"         Market conditions: 2022 bear market, high volatility decline")
    ...
    #Tuned hyperparameters from Ray Tune 
    config.model.hidden_dim = 64 #Optimal (which was confirmed from tuning)
    config.model.num_transformer_layers = 2
    config.model.num_attention_heads = 2 #Fewer heads prevent overfitting (was 4 before tuning)
    config.model.transformer_dropout = 0.226 #Moderate regularization
    config.model.lstm_dropout = 0.183 #Slightly less that before (was 0.2 before tuning)
    config.training.learning_rate = 0.000222 #Lower LR for more stability
    config.training.weight_decay = 0.00003116 #Optimal regularization
    config.training.batch_size = 192 #Larger batches hopefully for more stable gradients
    config.ensemble.weight_dropout = 0.237 #Optimal ensemble regularization
    config.training.num_epochs = 20 #Training epochs
    ...
    model, weight_stats = train_regime_aware_model(train_dataset, val_dataset, config)
    
    # 7. Generate regime-aware signals
    signals, regime_usage = generate_regime_aware_signals(model, test_features, symbols, sequence_length)
    ...
    if total_samples > 0:
        ...
        if scale_ratio > 10 or scale_ratio < 0.1:
            print(f"  ⚠️  WARNING: Predictions and actuals are on different scales!")
        ...
        print(f"  ℹ️  Note: Portfolio weights used original (unscaled) predictions for ranking")
    ...
    cost_scenarios = {
        'optimistic_weekly': {'bps': 5, 'freq': 'weekly'},
        'realistic_weekly': {'bps': 10, 'freq': 'weekly'},
        'pessimistic_weekly': {'bps': 20, 'freq': 'weekly'},
    }
    
    cost_labels = {
        'optimistic_weekly': '5 bps, weekly rebalance',
        'realistic_weekly': '10 bps, weekly rebalance',
        'pessimistic_weekly': '20 bps, weekly rebalance',
    }
    ...
    metrics_by_cost['optimistic'] = metrics_by_cost['optimistic_weekly']
    metrics_by_cost['realistic'] = metrics_by_cost['realistic_weekly']
    metrics_by_cost['pessimistic'] = metrics_by_cost['pessimistic_weekly']
