"""
ATTENTION-ONLY Multi-Asset Backtesting
Uses ONLY Attention-Enhanced Transformer (no vanilla/ensemble)
For comparison against ensemble and vanilla backtests
"""
...
OUTPUTS_DIR = PROJECT_ROOT / "outputs" / "attention_only"
...
from Models.Momentum_transformer import MomentumTransformerDualPath  # ATTENTION ONLY
...
print("=" * 80)
print("ATTENTION-ONLY MULTI-ASSET MOMENTUM TRANSFORMER")
print("=" * 80)
print("Using ONLY attention-enhanced transformer (no vanilla/ensemble)")
print("=" * 80)
...
def train_attention_model(train_dataset, val_dataset, config):
    """
    Train attention-only model (no vanilla, no ensemble)
    """
    print("\n[Step 4/8] Training Attention Model...")
    print("  Using ONLY attention-enhanced transformer")
    ...
    model = MomentumTransformerDualPath(
    ...
    for epoch in range(20):
        ...
        for X, y in train_loader:
            optimizer.zero_grad()
            predictions, _ = model(X)  # Returns (prediction, attention_info)
...
def generate_attention_signals(model, test_features, symbols, sequence_length=252):
    """
    Generate signals using attention model only
    """
    print("\n[Step 5/8] Generating Attention Signals...")
    
    model.eval()
    signals = {}
    regime_usage = defaultdict(lambda: {'vanilla': 0, 'attention': 1})  # All attention

    with torch.no_grad():
        for symbol in symbols:
            symbol_data = test_features[test_features['symbol'] == symbol].sort_values('timestamp')
            
            if len(symbol_data) < sequence_length:
                continue
            
            feature_cols = [c for c in symbol_data.columns 
                          if c not in ['symbol', 'timestamp', 'return']]
            
            # Use last sequence for prediction
            last_sequence = symbol_data[feature_cols].iloc[-sequence_length:].values
            X = torch.FloatTensor(last_sequence).unsqueeze(0)
            
            # Get attention prediction
            prediction, _ = model(X)
            signals[symbol] = float(prediction.squeeze().cpu().numpy())
    ...
    print(f"    Attention model: {len(signals)} stocks (100%)")
...
def save_performance_summary(metrics_by_cost: Dict[str, Dict],
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("ATTENTION-ONLY MULTI-ASSET PORTFOLIO PERFORMANCE SUMMARY\n")
        f.write("=" * 80 + "\n\n")
        ...
        for symbol, weight in sorted(portfolio_weights.items(), key=lambda x: abs(x[1]), reverse=True):
            ...
            f.write(f"  {symbol:6s}: {abs(weight)*100:>5.1f}% ({direction:>5s}) - Attention model\n")
        
        f.write("\n" + "=" * 80 + "\n")
        f.write("MODEL STATISTICS\n")
        f.write("-" * 80 + "\n")
        f.write(f"  Model: Attention-Enhanced Transformer (no vanilla/ensemble)\n")
        f.write(f"  → Using only attention model for all predictions\n")
...
def main():
    """Main attention-only multi-asset backtesting workflow"""
    ...
    # 6. Train attention model
    model = train_attention_model(train_dataset, val_dataset, config)
    
    # 7. Generate attention signals
    signals, regime_usage = generate_attention_signals(model, test_features, symbols, sequence_length)
    ...
    for symbol, weight in sorted(portfolio_weights.items(), key=lambda x: abs(x[1]), reverse=True):
        direction = "LONG" if weight > 0 else "SHORT"
        print(f"    {symbol:6s}: {abs(weight)*100:>5.1f}% ({direction}) - Attention model")
    ...
    save_performance_summary(metrics_by_cost, portfolio_weights, OUTPUTS_DIR, prediction_metrics)
    ...
    print("\n" + "=" * 80)
    print("✅ ATTENTION-ONLY MULTI-ASSET BACKTEST COMPLETE!")
    print("=" * 80)
    print(f"\nKEY FEATURES:")
    print(f"  ✅ Trained on {len(symbols)} symbols with stock-agnostic features")
    print(f"  ✅ Attention-enhanced transformer model (no vanilla/ensemble)")
    print(f"  ✅ Direct comparison against vanilla and ensemble models")
