"""
VANILLA-ONLY Multi-Asset Backtesting
Uses ONLY Vanilla Transformer (no attention/ensemble)
For comparison against ensemble backtest
"""
...
OUTPUTS_DIR = PROJECT_ROOT / "outputs" / "vanilla_only"
...
from Models.Momentum_transformer import MomentumTransformerSimple  # VANILLA ONLY
...
print("=" * 80)
print("VANILLA-ONLY MULTI-ASSET MOMENTUM TRANSFORMER")
print("=" * 80)
print("Using ONLY vanilla transformer (no attention/ensemble)")
print("=" * 80)
...
def train_vanilla_model(train_dataset, val_dataset, config):
    """
    Train vanilla-only model (no attention, no ensemble)
    """
    print("\n[Step 4/8] Training Vanilla Model...")
    print("  Using ONLY vanilla transformer")
...
def generate_vanilla_signals(model, test_features, symbols, sequence_length=252):
    """
    Generate signals using vanilla model only
    """
    print("\n[Step 5/8] Generating Vanilla Signals...")
    
    model.eval()
    signals = {}
    regime_usage = defaultdict(lambda: {'vanilla': 1, 'attention': 0})  # All vanilla
    ...
    print(f"    Vanilla model: {len(signals)} stocks (100%)")
...
def save_performance_summary(metrics_by_cost: Dict[str, Dict],
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("VANILLA-ONLY MULTI-ASSET PORTFOLIO PERFORMANCE SUMMARY\n")
        f.write("=" * 80 + "\n\n")
        ...
        for symbol, weight in sorted(portfolio_weights.items(), key=lambda x: abs(x[1]), reverse=True):
            ...
            f.write(f"  {symbol:6s}: {abs(weight)*100:>5.1f}% ({direction:>5s}) - Vanilla model\n")
        
        f.write("\n" + "=" * 80 + "\n")
        f.write("MODEL STATISTICS\n")
        f.write("-" * 80 + "\n")
        f.write(f"  Model: Vanilla Transformer (no attention/ensemble)\n")
        f.write(f"  → Using only vanilla model for all predictions\n")
...
def main():
    """Main vanilla-only multi-asset backtesting workflow"""
    ...
    # 6. Train vanilla model
    model = train_vanilla_model(train_dataset, val_dataset, config)
    
    # 7. Generate vanilla signals
    signals, regime_usage = generate_vanilla_signals(model, test_features, symbols, sequence_length)
    ...
    for symbol, weight in sorted(portfolio_weights.items(), key=lambda x: abs(x[1]), reverse=True):
        direction = "LONG" if weight > 0 else "SHORT"
        print(f"    {symbol:6s}: {abs(weight)*100:>5.1f}% ({direction}) - Vanilla model")
    ...
    print("\n" + "=" * 80)
    print("✅ VANILLA-ONLY MULTI-ASSET BACKTEST COMPLETE!")
    print("=" * 80)
    print(f"\nKEY FEATURES:")
    print(f"  ✅ Trained on {len(symbols)} symbols with stock-agnostic features")
    print(f"  ✅ Vanilla transformer model (no attention/ensemble)")
    print(f"  ✅ Direct comparison baseline for ensemble model")
